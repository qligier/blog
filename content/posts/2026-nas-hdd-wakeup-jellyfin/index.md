...
I have at home a NAS that I use as backup for important documents and as primary storage for my medias.
...
Prior to that, Jellyfin was running on an Intel NUC, which required a bit more maintenance (especially since it was
running Windows).

I am happy with this move, but one thing has been bothering me: the hard drive of the NAS are really slow to wake up.
...
All Docker containers and their data are stored on a SSD, while the bulk storage is on HDDs.
So when I browse the media library, the metadata is loaded from the SSD, but when I start a media, it has to be read
on the HDDs.
And those are often asleep (to preserve them and save energy).

I have searched how to wake up the HDDs when opening Jellyfin to prevent that delay, but haven't found a simple solution.
I have nonetheless found a workaround that is fast to implement and works well: I created a PHP script that
[stat](https://en.wikipedia.org/wiki/Stat_(system_call))s a directory on the HDDs (waking them up), and configured the
...
to expose it).
The script itself is stored on the SSD ('volume1'), which is mounted where the container expects it ('/var/www/html').
The other volume is giving read access to the media storage on the HDDs (that's the directory we'll stat).
...
It simply stats the directory on the HDDs, and clears the stat cache (that's a PHP thing) to make sure it hits the file
system.
